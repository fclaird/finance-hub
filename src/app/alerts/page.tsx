...
type Rule = { id: string; type: "drift" | "concentration"; enabled: boolean; config: any };
...
export default function AlertsPage() {
  ...
  async function saveRule(type: Rule["type"], enabled: boolean, config: any) {
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-zinc-200 p-4 dark:border-white/10">
            ...
            <div className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
              Threshold (%):{" "}
              <input
                className="ml-2 w-24 rounded-md border border-zinc-200 bg-white px-2 py-1 text-sm dark:border-white/10 dark:bg-zinc-950"
                value={((drift?.config?.thresholdPct ?? 0.05) * 100).toFixed(2)}
          ...
          <div className="rounded-xl border border-zinc-200 p-4 dark:border-white/10">
            ...
            <div className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
              ...
              <input
                className="ml-2 w-24 rounded-md border border-zinc-200 bg-white px-2 py-1 text-sm dark:border-white/10 dark:bg-zinc-950"
                value={((conc?.config?.maxSingleUnderlyingPct ?? 0.25) * 100).toFixed(2)}
                onChange={(e) =>
